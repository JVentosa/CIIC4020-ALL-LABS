...
public class KeysANDValuesINLinearProbingV1_Wrapper {

	public static int countNext; // DO NOT REMOVE THIS - TESTS WILL FAIL IF SO
	...
	/**
	 * Interface for Dictionary ADT. Search keys and associated values are not null.
	 **/
	public static interface DictionaryInterface<K, V> {
		...
		Iterable<V> values(); // iterable of values in the dictionary;
	} // end DictionaryInterface

	public static class LinearProbingHashingDictionary_V1<K, V> implements DictionaryInterface<K, V> {
		...
		private int findHashPos(K key) {
			...
			// Iterate until found or until determined that the key is not in the table
			// (which happens
			// when a null position is found in the table or the iteration process has gone
			// over all
			// the table once.
		...
		/**
		 * Checks if a given position (index h) in the table contains a given key.
		 * 
		...
		public V remove(K key) {
			...
			if (positionInTableContainsKey(index, key)) {
				...
				// is not needed if the position after (circular) is already null. THINK about
				// this.
		...
		private void increaseCapacity() {
			// Increase the capacity to at list the double of the current capacity. But make
			// sure
			...
			// the hashing indexes for each entry have automatically changed (because we
			// have
			// a new value for table.length). So, the entries in the old table must be
			// relocated
			// in the new table as per the new hashing indexes.
			// The algorithm is then to find the index where each entry must be located in
			// the
			// new table. For that, we apply the findPos method to the key of each entry.
			// Notice
		...
		// A simple brute-force algorithm to determine if a given integer is a prime
		// number.
		...
		private class EntryIterator implements Iterator<Entry<K, V>> {
			...
			public EntryIterator() {
				countNext = 0; // DO NOT REMOVE THIS - TESTS WILL FAIL IF SO
			}
		...
		// THE ABOVE IS THE INNER CLASSES FOR ITERABLE AND ITERATOR OF ENTRIES

		@Override
		public Iterable<Entry<K, V>> entries() {
			return new EntryIterable(); // see class EntryIterable
		}
		...
		// YOU NEED TO ADD THE IMPLEMENTATION OF YOUR ITERATOR AND ITERABLE CLASSES HERE
		// AS PER THE SPECIFICATIONSO OF THE LAB EXERCISE.
		// IN THIS CASE, TO ITERATE OVER THE VALUES IN THE DICTIONARY.

		// ADD YOUR CODE HERE -- JUST THE ADDITIONAL CODE NEEDED
		...
		private class ValueIterator implements Iterator<V> {
			...
			public V next() {
				...
				while (table[i] == null || table[i] == AVAILABLE) {
					i++;
				}

				V etr = table[i].getValue();
	...
	} // END OF INNER CLASS LinearProbingHashingDictionary_V1
